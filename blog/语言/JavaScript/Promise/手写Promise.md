...
2. 建立 MyPromise 静态常量, 三种状态 _PENDING_, _FULFILLED_, _REJECTED_
...
```js
it('异步resolve(1) ', (done) => {
    const promise = new MyPromise((resolve, reject) => setTimeout(resolve, 1, 1));
    promise.then((data) => {
        expect(data).toBe(1);
        done();
    });
...
```js
it('链式调用', (done) => {
    const promise = new MyPromise((resolve, reject) => setTimeout(resolve, 0, 1));
