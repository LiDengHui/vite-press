...
2.  **外边距折叠只发生在同一个 BFC 内：** 这是 BFC 最重要的隔离特性之一。**相邻块级元素**的垂直外边距（`margin-top` 和 `margin-bottom`）会发生折叠（Collapse）。**但是，如果它们属于不同的 BFC，它们的外边距就不会折叠。**
    - _例子：_ 两个相邻的 `<div>`，如果它们都在同一个 BFC（比如根元素 `<html>` 创建的 BFC）里，它们的垂直 `margin` 会重叠。如果其中一个 `<div>` 被包裹在一个新创建的 BFC（比如加了 `overflow: hidden`）中，那么这两个 `<div>` 的 `margin` 就不会重叠。
3.  **BFC 的区域不会与浮动元素重叠：** BFC 会识别并避开浮动元素。
    - _例子：_ 一个浮动元素（`float: left`）后面跟着一个普通流中的块级元素。普通流元素的内容默认可能会环绕在浮动元素周围（部分重叠）。如果给这个普通流元素创建 BFC（比如加 `overflow: auto`），它就会整个移动到浮动元素的右侧，形成真正的两栏布局，而不会重叠。
