...
在前端项目开发中，引入无内置 TypeScript 类型声明的第三方库时，TypeScript 编译器会报错。可在项目的 shims-vue.d.ts 中通过 declare 声明此模块来解决报错。同时引出几个问题：-为什么在 .shims-vue.d.ts 文件中写一个 declare 声明就能解决报错？
...
可在任意 .ts 文件中直接使用 **APP_VERSION**，无需引入。
